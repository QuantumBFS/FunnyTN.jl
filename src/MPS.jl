...
We use the following convention for numbering legs:
 1--A--3
    ...
    2
"""
type AbstractTN{T} end
type TensorTrain{T}<:AbstractTN{T} end

MPSTensor{T} = Array{T, 3}
MPOTensor{T} = Array{T, 4}

struct MPS{T} <: TensorTrain{T}
...
"""l-th bound of mps."""
bond(mps::MPS, l::Int) = Bond(rlink(mps.data[l]), llink(mps.data[l+1]))

import Base: push!, append!, prepend!, length, parent
